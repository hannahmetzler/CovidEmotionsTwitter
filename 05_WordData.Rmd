...
subtitle: "Supplementary Information 2: Word frequencies in tweets"
...
#add beginning character to each liwc term (otherwise book is matched with ok, for example)
dictpos <- paste0("^", dictpos)
...
```{r}
lowest_percent = round(range(c(min(anxb$Percent), min(anx$Percent), min(angb$Percent), min(ang$Percent), min(sadb$Percent), min(sad$Percent), min(posb$Percent), min(pos$Percent))),2)
```
...
No LIWC term was present in more than `r round(max(liwc$Percent), digits = 2)`% of tweets expressing a particular emotion. During the outbreak, panic buying and self-isolating were the two most frequent anxiety and sadness terms from LIWC. Percentages for all LIWC terms not listed in Table S2.1. were below `r lowest_percent[1]` to `r lowest_percent[2]` (frequencies of the least frequent words that made it into the top 60 per dictionary and time period).
...
#allow qual* only to match qualen, not e.g. qualität (mismatch)
dictanx = str_replace(dictanx, "qual", "qual$")
dictanx = c(dictanx, "qualen")
...
No LIWC term was present in more than `r round(max(liwc$Percent), digits = 2)`% of tweets expressing a particular emotion. Percentages for all LIWC terms not listed in Table S2.2. were below `r lowest_percent[1]` to `r lowest_percent[2]` (frequencies of the least frequent words that made it into the top 60 per dictionary and time period).
...
No LIWC term was present in more than `r round(max(liwc$Percent), digits = 2)`% of tweets expressing a particular emotion. Percentages for all LIWC terms not listed in Table S2.3. were below `r lowest_percent[1]` to `r lowest_percent[2]` (frequencies of the least frequent words that made it into the top 60 per dictionary and time period).
...
The 60 most frequent terms do not contain any of the LIWC terms, so all percentages were below `r lowest_percent[1]` to `r lowest_percent[2]` (frequencies of the least frequent words that made it into the top 60 per dictionary and time period).
...
Only one LIWC term  ("ongemak") was frequent enough (`r round(liwc$Percent, 2)`%) to be among the 60 most frequent terms. All other LIWC terms had a percentage below `r lowest_percent[1]` to `r lowest_percent[2]` percentages for the least frequent of the 60 terms per  dictionary and time period). 
...
No LIWC term was present in more than `r round(max(liwc$Percent), digits = 2)`% of tweets expressing a particular emotion.  Percentages for all LIWC terms not listed in Table S2.4. were below `r lowest_percent[1]` to `r lowest_percent[2]` (frequencies of the least frequent words that made it into the top 60 per dictionary and time period).
...
* Anger: déteste (hate) and gros/grosse (fat, serious, large) are important in the angry dictionary. They are are slighly less frequent during outbreak than during baseline. 
